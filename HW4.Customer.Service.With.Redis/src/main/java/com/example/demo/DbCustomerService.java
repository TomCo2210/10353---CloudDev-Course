...
public class DbCustomerService implements CustomerService {
	...
	@Override
	public Customer create(Customer customer) throws NotFoundException, BadDataException, AlreadyExistsException {
		if (customer == null)
		...
		Optional<Customer> optional = this.customers.findById(customer.getEmail()); // SELECT
		...
		if (Optionalcountry.isPresent())// update
			...
			country = new Country(customer.getCountry().getCountryCode(), customer.getCountry().getCountryName());
		}

		customer.setCountry(countries.save(country));
		countries.save(customer.getCountry());
		return this.customers.save(customer); // INSERT / SELECT + UPDATE = UPSERT
	}

	@Override
	public List<Customer> getAll(int size, int page) {
		return this.customers.findAll(PageRequest.of(page, size, Direction.ASC, "email")).getContent();
	}
	...
	public Customer getCustomerByEmail(String email) throws NotFoundException, BadDataException {
		...
		Optional<Customer> optional = this.customers.findById(email); // SELECT
	...
	public List<Customer> getCustomersByLastName(String lastName, int size, int page)
		...
		return this.customers.findAllByLast(lastName, PageRequest.of(page, size, Direction.ASC, "email", "email"));
		// SELECT
	}
	...
	public List<Customer> getCustomersByCountryCode(String countryCode, int size, int page)
		...
		return this.customers.findAllByCountryCountryCode(countryCode,
				PageRequest.of(page, size, Direction.ASC, "email", "email"));
		// SELECT
	}
	...
	public List<Customer> getCustomersByAgeGreaterThan(int age, int page, int size)
		...
		return this.customers.findByBirthdateBefore(date, PageRequest.of(page, size, Direction.ASC, "email", "email"));
	...
	public Customer update(String emailToUpdate, Customer update) throws NotFoundException, BadDataException {
		...
		if (update.getFirst() != null && update.getLast() != null) {
		...
		if (update.getCountry() != null)
			if (update.getCountry().getCountryCode() != null && update.getCountry().getCountryName() != null) {
				...
				if (Optionalcountry.isPresent())// update
					...
					country = new Country(update.getCountry().getCountryCode(), update.getCountry().getCountryName());
				rv.setCountry(countries.save(country));
			}
		return this.customers.save(rv); // SELECT INSERT / **UPDATE**
	}

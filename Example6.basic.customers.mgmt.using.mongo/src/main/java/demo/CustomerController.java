...
public class CustomerController {
	...
	public CustomerBoundary[] getAll (
		...
		if (byLastName.isEmpty()) {
			rv = this.customers.getAll(size, page, asc.equals(OrderEnum.ASC), sortBy.name().replace('_', '.'));
		}else {
			rv = this.customers.getByLastName(byLastName, size, page, asc.equals(OrderEnum.ASC), sortBy.name().replace('_', '.'));
		}
